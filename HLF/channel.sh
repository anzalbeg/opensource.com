...
kubectl exec $CLI_POD_ID --namespace org1namespace -it --  bash -c "CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH && peer channel create --logging-level=DEBUG -o $ORDERER_ADDR -c $CHANNEL_NAME -f $GENESIS_BLOCK"