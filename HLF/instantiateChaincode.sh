...
kubectl exec $CLI_POD_ID --namespace org1namespace -it -- bash -c "CORE_PEER_LOCALMSPID=$CORE_PEER_LOCALMSPID && CORE_PEER_MSPCONFIGPATH=$CORE_PEER_MSPCONFIGPATH && CORE_PEER_ADDRESS=$CORE_PEER_ADDRESS && CORE_VM_DOCKER_ATTACHSTDOUT=$CORE_VM_DOCKER_ATTACHSTDOUT && CORE_PEER_EVENTS_ADDRESS=$CORE_PEER_EVENTS_ADDRESS CORE_CHAINCODE_DEPLOYTIMEOUT=$CORE_CHAINCODE_DEPLOYTIMEOUT && CORE_CHAINCODE_STARTUPTIMEOUT=$CORE_CHAINCODE_STARTUPTIMEOUT && peer chaincode instantiate -o $ORDERER_ADDR -C $CHANNEL_NAME -n mycc -v 1.0 -c '{\"Args\":[\"init\",\"a\", \"100\", \"b\",\"200\"]}' -P \"OR ('Org1MSP.peer','Org2MSP.peer')\""
