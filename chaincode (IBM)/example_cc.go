...
// 	names := [...]string{"customer", "seller", "driver", "logisticArgument,", "logisticManager"}
// 	// prevent panicking in case of Weekday is out-of-range
// 	if n < customer || n > logisticManager {
// 		return "Unknown"
// 	}
...
func (t *SupplyChainChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	if function == "createShipment" { //create a new shipment
		...
		return t.createParticipantUser(stub, args)
	} else if function == "getParticipantUserbyID" { // create a participant within the organization
		return t.getParticipantUserbyID(stub, args)
...
func (t *SupplyChainChaincode) getOrganizationbyID(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	fmt.Println(_organizationID + " found")
...
func (t *SupplyChainChaincode) createOrganization(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	org.OrganizationID = "org" + fmt.Sprint(hash(org.Name))
	orgStruct := &Organization{org.OrganizationID, org.Tx, org.Name}
	_orgByte, _ := json.Marshal(orgStruct)
	fmt.Printf("organizationID----------------------------%s" + org.OrganizationID)
	err = stub.PutState(org.OrganizationID, _orgByte)
	if err != nil {
		return shim.Error(err.Error())
	}
	fmt.Println("- transaction created")

	tosend = "{  \"message\" : \"Organization created successfully on legder with organizationID \" : \"" + org.OrganizationID + "\", \"code\" : \"200\"}"
...
func (t *SupplyChainChaincode) getParticipantUserbyID(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var jsonResp, errResp string
	fmt.Println("start getParticipantUserbyID", args[0])
	var err error
	if len(args) != 1 {
		errMsg := "{ \"message\" : \"Incorrect number of arguments. Expecting 1\" \" as an argument\", \"code\" : \"503\"}"
		err = stub.SetEvent("errEvent", []byte(errMsg))
	...
	_participantID := args[0]

	fmt.Println(_participantID + " found")

	_tempJSON := ParticipantUser{}

	jsonResp = ""
	valueAsBytes, err := stub.GetState(_participantID)
	if err != nil {
		errResp = "{\"Error\":\"Failed to get state for " + _participantID + "\"}"
		return shim.Error(errResp)
	}
	json.Unmarshal(valueAsBytes, &_tempJSON)

	if _tempJSON.ParticipantID == _participantID {
		jsonResp = string(valueAsBytes[:])
	} else {
		fmt.Println(_participantID + " not found")
		errMsg := "{ \"Participant User\" : \"" + _participantID + "\",\"message\" : \"" + "Participant User Not Found.\", \"code\" : \"503\"}"
		err = stub.SetEvent("errEvent", []byte(errMsg))
		if err != nil {
			return shim.Error(err.Error())
		}
	...
	fmt.Println("jsonResp : " + jsonResp)
	fmt.Println("end getparticipantUserbyID")
	return shim.Success([]byte(jsonResp))
	//send it onward
}

func (t *SupplyChainChaincode) createParticipantUser(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	var tosend string
	...
	org, err = stub.GetState(user.Organization)
	if err != nil {
		return shim.Error("Failed to get Organization with organizationID " + user.Organization)
	} else if org == nil {
		return shim.Error("Organization with organizationID " + user.Organization + "does not exist")
	}

	user.ParticipantID = "user" + fmt.Sprint(hash(user.Name))
	userStruct := &ParticipantUser{user.ParticipantID, user.Tx, user.Name, user.Organization, user.Role}
	_userByte, _ := json.Marshal(userStruct)
	fmt.Printf("participantID----------------------------%s" + user.ParticipantID)
	err = stub.PutState(user.ParticipantID, _userByte)
	if err != nil {
		return shim.Error(err.Error())
	}
	fmt.Println("- transaction created")

	tosend = "{  \"message\" : \"Participant User created successfully on legder with participantID \" : \"" + user.ParticipantID + "\", \"code\" : \"200\"}"
	//}else if count == len(existPolicyArray){
	//tosend = "{ \"message\" : \"All of these Polices already exists in legder.\", \"Policies failed to stored on ledger with policyIds \" : \"" + strings.Join(existPolicyArray, ",") + "\", \"code\" : \"503\"}"
	//}else{
	//tosend = "{ \"message\" : \"Polices stored successfully on legder except few policies which already exists in ledger. Please find below details for more information.\", \"Policies successfully stored on ledger with policyIds \" : \"" + strings.Join(policyIdArray, ",") + "\", \"Policies failed to stored on ledger with policyIds \" : \"" + strings.Join(existPolicyArray, ",") + "\", \"Properties successfully created/updated with propertyIds\" : \"" + strings.Join(propertyIdArray, ",") + "\", \"code\" : \"503\"}"
...
func (t *SupplyChainChaincode) assignSecurityRole(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	var tosend string
	var userTemp, user ParticipantUser
	var userAsBtytes []byte
	...
	userAsBtytes, err = stub.GetState(user.ParticipantID)
	if err != nil {
		return shim.Error("Failed to get Participant User with participantID " + user.ParticipantID)
	} else if userAsBtytes == nil {
		return shim.Error("Participant User with participantID " + user.ParticipantID + "does not exist")
	}

	err = json.Unmarshal([]byte(userAsBtytes), &userTemp)
	if err != nil {
		log.Fatal(err)
	}

	userStruct := &ParticipantUser{userTemp.ParticipantID, userTemp.Tx, userTemp.Name, userTemp.Organization, user.Role}
	_userByte, _ := json.Marshal(userStruct)
	err = stub.PutState(user.ParticipantID, _userByte)
	if err != nil {
		return shim.Error(err.Error())
	...
	tosend = "{  \"message\" : \"Participant User role successfully updated with new role \" : \"" + user.Role + "\", \"code\" : \"200\"}"
	//}else if count == len(existPolicyArray){
	//tosend = "{ \"message\" : \"All of these Polices already exists in legder.\", \"Policies failed to stored on ledger with policyIds \" : \"" + strings.Join(existPolicyArray, ",") + "\", \"code\" : \"503\"}"
	//}else{
	//tosend = "{ \"message\" : \"Polices stored successfully on legder except few policies which already exists in ledger. Please find below details for more information.\", \"Policies successfully stored on ledger with policyIds \" : \"" + strings.Join(policyIdArray, ",") + "\", \"Policies failed to stored on ledger with policyIds \" : \"" + strings.Join(existPolicyArray, ",") + "\", \"Properties successfully created/updated with propertyIds\" : \"" + strings.Join(propertyIdArray, ",") + "\", \"code\" : \"503\"}"
