...
var createChannel = async function(channelName, channelConfigPath, configUpdate, username, orgName) {
	...
	try {
		...
		logger.debug('channel config path is****************"', path.join(__dirname, channelConfigPath));
		...
		logger.debug('signature', JSON.stringify(signature));
		...
		if (configUpdate){
			...
			response = await client.updateChannel(request);
		}
